        ...
        <ul className="divide-y divide-[var(--ps-border-soft)]">
          {ACTIVITY.map(item => (
            <li key={item.id} className="flex items-center gap-4 px-6 py-3.5">
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-full text-[14px] ${item.tint}`}
              >
                {item.glyph}
              </span>
...
const ACTIVITY = [
  { id: 1, glyph: '📄', tint: 'bg-blue-50',           title: 'Facture PS-1031 envoyée',               date: 'Aujourd’hui · 14:22' },
  { id: 2, glyph: '✅', tint: 'bg-emerald-50',        title: 'Maquette homepage validée',              date: 'Hier · 17:08' },
  { id: 3, glyph: '🎨', tint: 'bg-[var(--ps-primary-subtle)]', title: 'Charte graphique livrée',       date: '12 mai' },
  { id: 4, glyph: '📞', tint: 'bg-amber-50',          title: 'Appel découverte planifié',              date: '10 mai' },
];
