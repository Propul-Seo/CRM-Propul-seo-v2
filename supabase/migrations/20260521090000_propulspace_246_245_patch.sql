-- Patch RPC admin_convert_qualif_to_project suite code review BLOC 2 :
-- - Suppression v_notes (jamais inséré, code mort en prod — réintégration prévue
--   après ajout colonne projects_v2.notes dans session future)
-- - Numérotation screenshots dédiée (v_screenshot_idx) au lieu du compteur global
--   v_docs_count (évitait "Capture site existant #4" pour le 1er screenshot)
CREATE OR REPLACE FUNCTION propulspace.admin_convert_qualif_to_project(
  p_qualif_id uuid, p_activate_portal boolean DEFAULT false
)
...
BEGIN
  ...
  INSERT INTO public.projects_v2 (
    ...
    progress, is_archived, portal_client_email, siret, company_data
  ) VALUES (
