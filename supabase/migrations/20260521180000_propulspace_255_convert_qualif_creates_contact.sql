-- ============================================================================
-- Migration 255 — Conversion lead→projet crée aussi le contact CRM
-- ============================================================================
-- Manquait à la RPC : créer un contact (role 'primary') et le lier au projet.
-- Sans ça, l'admin doit re-saisir manuellement nom/email/téléphone/entreprise
-- alors que tout vient déjà du questionnaire.
--
-- Idempotent : skip la création si un contact 'primary' existe déjà sur le
-- projet (au cas où la RPC serait rejouée après crash partiel ou backfill).
-- ============================================================================
...
BEGIN
  ...
  -- Création contact primary depuis les infos questionnaire (idempotent).
