-- ============================================================================
-- Migration 253 — Fix conversion lead→projet : ne pas remplir portal_client_email
-- ============================================================================
-- Bug E2E observé : la RPC admin_convert_qualif_to_project (BLOC 2, migration
-- 245) remplissait directement portal_client_email lorsque p_activate_portal=true.
-- L'edge function admin-portal-invite, appelée juste après par le front,
-- refusait alors avec 409 "Portail déjà actif" (check ligne 105 de l'edge).
-- Résultat : projet créé en état "orphan" (email présent, pas d'invitation).
--
-- Fix : laisser portal_client_email NULL dans la RPC. L'edge function reste
-- responsable de remplir le champ après envoi de l'invitation Supabase Auth.
-- ============================================================================
...
BEGIN
  ...
  -- FIX 253 : portal_client_email NULL ici, l'edge admin-portal-invite s'en charge.
  ...
  RETURN jsonb_build_object('project_id', v_project_id, 'documents_created', v_docs_count, 'portal_activated', p_activate_portal);
END;
$function$;
