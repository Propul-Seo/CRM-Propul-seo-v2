-- ============================================================================
-- Migration 256 — Conversion lead→projet crée aussi une activité auto
-- ============================================================================
-- Ajout : la RPC admin_convert_qualif_to_project insère une ligne dans
-- public.project_activities_v2 (type='system', is_auto=true) pour tracer
-- "Questionnaire de qualification rempli le {submitted_at}" dans la timeline.
-- Idempotent : skip si une activité metadata.source LIKE 'qualif_conversion%'
-- existe déjà sur le projet (au cas où la RPC serait rejouée).
--
-- Backfill inclus à la fin : crée l'activité pour les projets déjà convertis.
-- ============================================================================
...
-- Backfill activités pour projets déjà convertis sans activité qualif.
INSERT INTO public.project_activities_v2 (project_id, user_id, author_name, type, content, is_auto, metadata, created_at)
SELECT
  ql.converted_to_project_id, NULL, 'Système', 'system',
  'Questionnaire de qualification rempli le ' || to_char(COALESCE(ql.submitted_at, ql.converted_at), 'DD/MM/YYYY HH24:MI') ||
  '. Type : ' || COALESCE(ql.project_type, 'site') || '. Voir l''onglet Brief / questionnaire pour le détail.',
  true,
  jsonb_build_object('source', 'qualif_conversion_backfill', 'qualif_id', ql.id, 'submitted_at', ql.submitted_at),
  COALESCE(ql.converted_at, now())
FROM public.qualification_leads_v2 ql
WHERE ql.converted_to_project_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.project_activities_v2 pa
    WHERE pa.project_id = ql.converted_to_project_id
      AND pa.metadata->>'source' LIKE 'qualif_conversion%'
  );
