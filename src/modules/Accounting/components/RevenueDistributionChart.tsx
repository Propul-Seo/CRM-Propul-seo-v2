  ...
  return (
    <div className="min-w-[210px] rounded-xl border border-white/14 bg-[#0d0a17]/95 px-4 py-3 text-violet-50 shadow-[0_18px_50px_rgba(0,0,0,0.45)] backdrop-blur-xl">
      ...
      <div className="flex items-end justify-between gap-4">
        <p className="text-xl font-black tracking-tight text-cyan-200">
...
export function RevenueDistributionChart({ chartData, communicationChartData, categoryFilter }: RevenueDistributionChartProps) {
  ...
  const title = categoryFilter === 'communication' ? 'Répartition autres sources' : 'Répartition par catégorie';

  if (data.length === 0) {
    return (
      <div className="relative flex h-[340px] items-center justify-center overflow-hidden rounded-2xl border border-violet-500/18 bg-[linear-gradient(180deg,rgba(17,13,29,0.96),rgba(8,7,14,0.98))] p-5">
        <p className="text-sm text-violet-100/55">Aucune donnée à afficher</p>
      </div>
    );
  }

  return (
    <div className="relative overflow-hidden rounded-2xl border border-violet-500/18 bg-[radial-gradient(circle_at_50%_0%,rgba(34,211,238,0.08),transparent_34%),linear-gradient(180deg,rgba(17,13,29,0.96),rgba(8,7,14,0.98))] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.2)]">
      ...
      <div className="mb-2 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-violet-100/46">Poids des sources filtrées</p>
        </div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <PieChart margin={{ top: 12, right: 18, bottom: 8, left: 18 }}>
            ...
            <text x="50%" y="47%" textAnchor="middle" dominantBaseline="middle" fill="rgba(237,233,254,0.92)">
              ...
              <tspan x="50%" dy="1.55em" fontSize="14" fontWeight="800" fill="#67e8f9">
                {formatCurrency(total)}
              </tspan>
            </text>
          </PieChart>
